...
def generate_rag_response(query):
    ...
    prompt = f"""
            أنت باحث أكاديمي وخبير استشاري. مهمتك هي تقديم إجابة مفصلة، دقيقة، وشاملة بناءً على "المقاطع النصية المستخرجة" التالية فقط.

            تعليمات الإجابة:
            1. استعن بالحقائق، الأرقام، والأمثلة المذكورة في المقاطع لدعم إجابتك بشكل مباشر (مثل أوزان الحجارة، التواريخ، أو النظريات).
            2. قم بصياغة الإجابة بأسلوب احترافي، واضح، ومترابط. استخدم الفقرات والنقاط (Bullet points) لتسهيل القراءة.
            3. لا تكتفِ بنسخ النصوص ولصقها؛ بل قم بصياغتها بأسلوبك مع الحفاظ التام على المعنى الأصلي.
            4. إذا لم تكن المعلومات كافية للإجابة على السؤال بشكل كامل، أجب بما يتوفر لديك ثم أضف ملاحظة: "المعلومات المتوفرة في التسجيل الصوتي لا تغطي هذا الجانب بالكامل".

            المعلومات المستخرجة من التسجيل الصوتي:
            {context_text}
